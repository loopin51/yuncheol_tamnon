...
# 4. Morphological Opening: 과도하게 큰 별의 영역을 줄이고 노이즈 제거
kernel = np.ones((10, 10), np.uint8)
opened_image = cv2.morphologyEx(thresholded_image, cv2.MORPH_OPEN, kernel)

# 5. Blob Detection을 위한 파라미터 설정
...
params.maxArea = 50000  # 큰 별도 감지될 수 있도록 값 크게 설정
...
# 감지된 별(블롭)을 추출
keypoints = detector.detect(opened_image)

# 6. 별의 위치와 크기 인덱싱 및 흑백 이미지에 별 표시
